...
XMLhttp.prototype = {
	...
	doscript : function() {    for (var i = 0; i < this.sArray.length; i++) {        var id = path = code = '';        if (this.sArray[i]['attribute'].match(/\s*id\="([\w\_]+?)"/i)) {            id = RegExp.$1;        }        if (this.sArray[i]['attribute'].match(/\s*src\="(.+?)"/i)) {            path = RegExp.$1;        } else {            code = this.sArray[i]['code'];        }        loadjs(path, code, id);    }    this.sArray = new Array();},
